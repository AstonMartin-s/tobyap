...
// Chime in-page: el SO a veces no suena si el chat está abierto (Chrome focused /
// iOS con PWA visible). El Web Push igual pinta el aviso; esto cubre el audio.
...
export default function ChatWidget({ slug, token, campaign, ccpp, brand, primaryColor, avatarUrl }: { slug: string; token: string; campaign: string; ccpp: string; brand: string; primaryColor?: string; avatarUrl?: string | null }) {
  ...
  const [skin, setSkin] = useState<{ brand: string; primaryColor?: string; avatarUrl?: string | null; gate?: { title: string; note: string; confirm: string } }>({ brand, primaryColor, avatarUrl });
  ...
  useEffect(() => {
    fetch(`/api/chat/${slug}/brand`)
      ...
      .then((d) => {
        if (!d?.brand) return;
        setSkin({ brand: d.brand.brandName, primaryColor: d.brand.primaryColor, avatarUrl: d.brand.avatarUrl, gate: d.brand.gate });
    ...
    const t = setInterval(async () => {
      try {
        ...
        if (d.total > pollBase.current) {
          ...
          // Fallback: si el Web Push no llegó (sin sub / VAPID / iOS in-tab),
          // el poll sigue avisando con el chat abierto. Si el SW ya pingeó, no
          // repetimos para no duplicar sonido.
          if (Date.now() - lastPushAt.current > 4000 && typeof Notification !== 'undefined' && Notification.permission === 'granted' && typeof navigator !== 'undefined' && 'serviceWorker' in navigator) {
    ...
    <div style={{ position: 'fixed', inset: 0, height: '100dvh', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, sans-serif', background: C.bg, paddingTop: 'env(safe-area-inset-top, 0px)', paddingBottom: 'env(safe-area-inset-bottom, 0px)', boxSizing: 'border-box' }}>
      <div style={{ background: header, color: '#fff', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 3px rgba(0,0,0,.2)' }}>
        ...
        {phase === 'chat' && waBtn.enabled && waBtn.url && (
          (waUnlocked || step === 'done') && effectiveWaUrl ? (
            <a
              href={effectiveWaUrl}
              target="_blank"
              rel="noreferrer"
              className="wa-hdr wa-hdr--fast"
              title="Ir a WhatsApp"
              style={{ display: 'flex', alignItems: 'center', gap: 6, border: 'none', cursor: 'pointer', background: '#25D366', color: '#fff', flexShrink: 0, borderRadius: 20, padding: '7px 14px 7px 10px', fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', textDecoration: 'none' }}
            >
            ...
            <div style={{ maxWidth: '80%', background: m.from === 'user' ? C.userBubble : C.botBubble, color: '#111827', padding: '7px 10px', borderRadius: 10, borderTopLeftRadius: m.from === 'bot' ? 2 : 10, borderTopRightRadius: m.from === 'user' ? 2 : 10, boxShadow: '0 1px 1px rgba(0,0,0,.12)', fontSize: 15, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
              {m.image ? (
                ...
                m.copy ? (
                  ...
                  <div>
                    <div style={{ marginBottom: 8 }}>{renderText(m.text ?? '')}</div>
                    <a href={m.wa} target="_blank" rel="noreferrer"
                      style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#25D366', color: '#fff', borderRadius: 10, padding: '10px 14px', fontSize: 15, fontWeight: 700, textDecoration: 'none', width: '100%', textAlign: 'center', lineHeight: 1.3 }}>
                      <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" style={{ flex: '0 0 auto' }}><path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.9 9.9 0 004.79 1.22h.01c5.46 0 9.9-4.45 9.9-9.91C21.95 6.45 17.5 2 12.04 2zm0 18.15h-.01a8.2 8.2 0 01-4.18-1.14l-.3-.18-3.11.82.83-3.04-.2-.31a8.19 8.19 0 01-1.26-4.39c0-4.54 3.7-8.23 8.24-8.23 2.2 0 4.27.86 5.83 2.42a8.18 8.18 0 012.42 5.82c0 4.54-3.7 8.23-8.24 8.23zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.12-.16.25-.64.81-.79.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.01-.38.11-.51.11-.11.25-.29.37-.43.12-.14.16-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.4-.42-.56-.43h-.48c-.17 0-.43.06-.66.31-.23.25-.87.85-.87 2.07 0 1.22.89 2.4 1.01 2.56.12.17 1.75 2.67 4.23 3.74.59.26 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.07.14-1.18-.06-.1-.22-.16-.47-.28z"/></svg>
                      {m.waLabel || 'Abrir WhatsApp'}
                    </a>
                  </div>
                ) : renderText(m.text ?? '')
              )}
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.45)', display: 'grid', placeItems: 'center', padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 22, width: '100%', maxWidth: 380, boxShadow: '0 10px 40px rgba(0,0,0,.3)' }}>
            ...
            <label style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 13, color: '#111827', margin: '6px 0 14px', cursor: 'pointer' }}>
              ...
              <span>{skin.gate?.confirm ? skin.gate.confirm : niche === 'tienda' ? <>Confirmo que este es <b>mi número</b>. Si no lo es, no podré recibir mi compra.</> : <>Confirmo que este es <b>mi número</b>. Si no lo es, no podré recibir mi bonificación.</>}</span>
            </label>
