...
async function save(tenant: string, day: string, gasto: number, ingreso: number) {
  const res = await fetch('/api/admin/ledger', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ tenant, day, gasto, ingreso }),
  });
...
function recalc(r: DailyRow): DailyRow {
  return {
    ...
    balance: +(r.ingreso - r.gasto).toFixed(2),
  };
}
...
export function DailyReportClient({ initial }: { initial: DailyRow[] }) {
  ...
  async function clear(r: DailyRow) {
    ...
    try {
      ...
      setRows((prev) => prev.map((x) => (key(x) === key(r) ? recalc({ ...x, gasto: 0, ingreso: 0 }) : x)));
    } catch (e) { setErr((e as Error).message); }
    finally { setSaving(null); }
  }

  const tot = rows.reduce((a, r) => ({ chats: a.chats + r.chats, cargas: a.cargas + r.cargas, gasto: a.gasto + r.gasto, ingreso: a.ingreso + r.ingreso }), { chats: 0, cargas: 0, gasto: 0, ingreso: 0 });
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            <th className="num">Chats</th><th className="num">Cargas</th>
            <th className="num">Gasto</th><th className="num">Depósitos</th>
            <th className="num">$/Chat</th><th className="num">$/Carga</th>
            <th className="num">Conv.</th><th className="num">Balance</th><th></th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => {
            ...
            return (
              <tr key={k}>
                ...
                <td className="num">
                  <input className="input input--cell" type="number" step="0.01" value={r.ingreso || ''} placeholder="0.00"
                    onChange={(e) => setField(k, 'ingreso', Number(e.target.value))}
                    onBlur={() => persist(r)} />
                </td>
                ...
                <td className="num" style={{ color: r.balance >= 0 ? 'var(--accent)' : 'var(--danger)' }}>{money(r.balance)}</td>
                <td className="num" style={{ width: 28 }}>
                  {saving === k ? <span className="spinner" style={{ width: 14, height: 14 }} />
                    : (r.gasto || r.ingreso) ? <button className="btn btn--danger-ghost btn--sm" title="Eliminar valores" onClick={() => clear(r)}>✕</button> : null}
                </td>
              </tr>
            );
          })}
        ...
        <tfoot>
          <tr style={{ borderTop: '2px solid var(--border-2)', fontWeight: 700 }}>
            ...
            <td className="num">{money(tot.ingreso)}</td>
            ...
            <td className="num" style={{ color: tot.ingreso - tot.gasto >= 0 ? 'var(--accent)' : 'var(--danger)' }}>{money(tot.ingreso - tot.gasto)}</td>
            <td></td>
          </tr>
        </tfoot>
      </table>
